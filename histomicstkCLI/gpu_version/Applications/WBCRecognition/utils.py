...
large_image.cache_util.cachefactory.defaultConfig[
...
def create_tile_cell_bbox_annotations(cell_obj_props, tile_info):
    ...
    for i in range(len(cell_obj_props)):
        cx = cell_obj_props[i]['center'][0]
        cy = cell_obj_props[i]['center'][1]
        width = cell_obj_props[i]['width'] + 1
        height = cell_obj_props[i]['height'] + 1

        class_color = cell_obj_props[i]['lineColor']
        ...
        cur_bbox = {
            ...
            "lineColor": class_color
        }

        cell_annot_list.append(cur_bbox)

    return cell_annot_list
...
def create_tile_cell_annotations(im_nuclei_seg_mask, tile_info, format):

    if format == 'bbox':

        return create_tile_cell_bbox_annotations(im_nuclei_seg_mask,
                                                 tile_info)
